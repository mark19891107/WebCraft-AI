...
| `/data` | 資料來源管理：上傳/貼上 JSON/預覽/刪除 CSV/JSON（存 OPFS，支援 2GB+）|
...
- ✅ **資料摘要改結構感知截斷**：`summarizeData` 不再對整段字串做 800 字硬切，改為遞迴截斷（長字串值縮短、陣列限制取樣筆數+備註總數、限制巢狀深度），即使第一筆資料很大也保留所有欄位名；最終仍有總長度上限當保險。新增測試（共 34）。
- ✅ **腦力激盪題型判斷準則**：prompt 補上 single/multi/text 的明確判斷準則（互斥選項→單選、可複選需求→複選、開放式→文字），提升 LLM 判斷一致性。
- ✅ **資料來源：貼上 JSON**：`/data` 頁新增「貼上 JSON」按鈕，開 Modal 輸入檔名+貼上 JSON 文字，前端驗證合法 JSON 後寫入 OPFS（沿用既有上傳/預覽/刪除流程），免去先存成檔案再上傳的步驟。
